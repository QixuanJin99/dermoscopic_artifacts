...
def get_lora_sd_img2img_pipeline(
    ...
    # Use StableDiffusionImg2ImgPipeline for img2img capability
...
if args.dataset == "ISIC": 
    ...
    full_dataset = PH2Dataset(df, image_dir, mode=dataset_mode, return_pil=True)
elif args.dataset == "BCN20000": 
...
pipe = get_lora_sd_img2img_pipeline(ckpt_dir = Path(args.ckpt_dir),
                            base_model_name_or_path="runwayml/stable-diffusion-v1-5", 
                            adapter_name=args.keyword)

batch_size = 3  # Num duplicates per image 
...
with ThreadPoolExecutor() as executor:  # Use parallel saving
    for batch_start in range(370, num_images, batch_size):
        
        init_image = full_dataset[int(batch_start//batch_size)][0].resize((512, 512), Image.BILINEAR)
        images = pipe(prompt, 
                  image=init_image,
        ...
        for i, img in enumerate(images):
            img_path = os.path.join(save_dir, f"img_{batch_start + i + 1}.png")
